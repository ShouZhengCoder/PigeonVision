...
本文档记录信鸽虹膜识别系统从初始 35.5% Search Hit@1 提升到 70.9%（全量）/ 68.0%（干净切分）的完整过程，包括问题诊断、对照实验和关键决策。
...
| 实验 | 训练数据 | Hit@1 | mAP | AUC |
|------|------|:---:|:---:|:---:|
| A (起点) | Triplet, split | 35.5% | 16.2% | 73.1% |
| B | **Relation-SupCon, split (40 ep)** | **52.1%** | 38.1% | **91.3%** |
| C | Relation-SupCon, split (300 ep) | **68.0%** | **54.9%** | 87.4% |
| D | Relation-SupCon, 全量 (60 ep) | 70.9% | 60.7% | 99.5% |

- 损失函数改进（B - A）：Hit@1 +16.6%, AUC +18.2%
- 充分训练增益（C - B）：Hit@1 +15.9%, 但 AUC -3.9%
- 全量数据增益（D - C）：Hit@1 +2.9%, AUC +12.1%
...
干净切分训练 300 epoch 后 loss 降至 0.72，Hit@1 从 52.1% 持续提升至 68.0%。
...
| 指标 | 初始 | 干净切分 | 全量训练 |
|------|:---:|:---:|:---:|
| Search Hit@1 | 35.5% | **68.0%** | 70.9% |
| Search Hit@5 | — | **75.5%** | 83.8% |
| Search Hit@10 | 59.1% | **78.1%** | 87.8% |
| Search mAP | 16.2% | **54.9%** | 60.7% |
| Compare AUC | 73.1% | **87.4%** | 99.5% |
| Compare BalAcc | — | **84.0%** | 97.0% |

干净切分下 Hit@1 从 35.5% 提升到 68.0%（+32.5 个百分点）。
